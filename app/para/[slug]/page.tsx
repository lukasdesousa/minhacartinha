...
export async function generateMetadata({ params }: PageProps<"/para/[slug]">): Promise<Metadata> {
  ...
  if (!letter) {
    return { title: "Cartinha não encontrada | Minha Cartinha" };
  }

  return {
    title: `Uma cartinha para ${letter.recipientName} | Minha Cartinha`,
    description: `${letter.senderName} preparou uma cartinha especial para ${letter.recipientName}.`,
  };
}

export default async function PublicLetterPage({ params }: PageProps<"/para/[slug]">) {
  const { slug } = await params;
  const letter = await getPublishedLetterBySlug(slug);
