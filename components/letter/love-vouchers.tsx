...
export function LoveVouchers({ slug, vouchers, demo = false }: { slug: string; vouchers: PublicLoveVoucher[]; demo?: boolean }) {
  ...
  return <section className="bg-[var(--letter-wash)] px-5 py-20 sm:px-8 sm:py-24" aria-labelledby="love-vouchers-title">
    ...
    <div className={`mx-auto mt-10 grid gap-5 ${items.length === 1 ? "max-w-lg grid-cols-1" : items.length === 2 ? "max-w-3xl sm:grid-cols-2" : "max-w-5xl sm:grid-cols-2 lg:grid-cols-3"}`}>{items.map((voucher, index) => {
